...
namespace ETicaretAPI.Persistence.Services
{
    public class AuthService : IAuthService
    {
        ...
        public AuthService(HttpClient httpClient, UserManager<Domain.Entities.Identity.AppUser> userManager, IConfiguration configuration, ITokenHandler tokenHandler, SignInManager<AppUser> signInManager, IUserService userService)
        {
        ...
        async Task<Token> CreateUserExternalAsync(AppUser user, string email, string name, UserLoginInfo info, int accessTokenLifeTime)
        {
            ...
            if (result)
            {
                ...
                await _userService.UpdateRefreshToken(token.RefreshToken, user, token.Expration.ToUniversalTime(), 5);
                return token;
            }
        ...
        public async Task<Token> FacebookLoginAsync(string authToken, int accessTokenLifeTime)
        {
            ...
            string userAccesTokenValidation = await _httpClient.GetStringAsync($"https://graph.facebook.com/debug_token?input_token={authToken}&access_token={facebookAccessTokenResponse?.AccessToken}");
        ...
        public async Task<Token> RefreshTokenLoginAsync(string refreshToken)
        {
            ...
            if(user != null && user?.RefreshTokenEndDate > DateTime.UtcNow)
            {
               Token token =  _tokenHandler.CreateAccessToken(15, user);
                await _userService.UpdateRefreshToken(token.RefreshToken, user, token.Expration, 15);
                return token;
            }
            else
            {
